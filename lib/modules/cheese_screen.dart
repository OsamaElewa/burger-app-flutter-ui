...
class CheeseScreen extends StatelessWidget {
   CheeseScreen({Key? key}) : super(key: key);

   List<ItemModel> cheeselst=itemList.where((element) => element.catId==4).toList();
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      childAspectRatio: .76,
      children:
      List.generate(cheeselst.length,
              (index) => buildItem(cheeselst[index],context)
      ),
    );
  }
}
