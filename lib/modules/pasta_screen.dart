...
class PastaScreen extends StatelessWidget {
   PastaScreen({Key? key}) : super(key: key);

   List<ItemModel> pastalst=itemList.where((element) => element.catId==3).toList();
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      childAspectRatio: .76,
      children:
      List.generate(pastalst.length,
              (index) => buildItem(pastalst[index],context)
      ),
    );
  }
}
